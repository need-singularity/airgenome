...
**Status**: rebuild v2 진행 중 (2026-04-13 동결). SSOT: [`shared/config/roadmap/airgenome.json`](shared/config/roadmap/airgenome.json).
...
hexa run ~/Dev/nexus/shared/lockdown/l0_guard.hexa verify

# probe — Mac+ubu+htz vitals → nexus/shared/infra_state.json
...
| M2  | probe — Mac+ubu+htz vitals → infra_state | P1       | 🔄 active | M0, M1   | airgenome#37 · nexus#36 (L0 promote) |
| M3  | dispatch — infra_state → best host (AG6/AG7) | P1   | 🔄 active | M2       | airgenome#39                         |
| M4  | harvest — 60-byte hexagon per process    | P1       | 📝 todo   | M2       | —                                    |
| M5  | label — anomaly → behavior 라벨 (T15)    | P2       | 📝 todo   | M4       | —                                    |
| M6  | predict — 7d 추세 → 1h 예측              | P3       | 📝 todo   | M4       | —                                    |
